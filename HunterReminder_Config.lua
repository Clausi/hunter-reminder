...
addonData.config.initConfig = function()
    ...
    panel.mainConfig =
        addonData.config.createConfigPanel(
        panel,
        "mainConfig",
        {width = 1, height = 1},
        {relativePoint = "TOPLEFT", xOfs = 10, yOfs = -10}
    )
    InterfaceOptions_AddCategory(panel)

    panel.configPetPanel = CreateFrame("Frame", nil, panel)
    panel.configPetPanel:SetSize(1, 1)
    panel.configPetPanel.petPanel =
        addonData.config.createConfigPanel(
        panel.configPetPanel,
        "petConfig",
        {width = 1, height = 1},
        {relativePoint = "TOPLEFT", xOfs = 10, yOfs = -10}
    )
...
addonData.config.configPanelFactory = function(parentPanel, panelName, size, point)
    ...
    -- Title Text
    panel.titleText =
        addonData.config.text(
        panel,
        "General config",
        20,
        {relativePoint = "TOPLEFT", xOfs = 0, yOfs = 0},
        {r = 1, g = 0.9, b = 0, a = 1}
    )
